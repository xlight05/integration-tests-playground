...
function testNormalPackage() returns error? {
    ...
    //API
    Package packageResp = check centralApiClient -> get("registry/packages/bctestorg/normal_package/0.1.0");
    ...
    test:assertEquals(packageResp.URL, "/bctestorg/normal_package/0.1.0");
    test:assertEquals(packageResp.visibility, "public");
}

@test:Config { }
function testNormalPackageApiDocs() returns error? {
    http:Response resp = check apiDocsClient -> get("bctestorg/normal_package/0.1.0");
    test:assertEquals(resp.statusCode, 200);

    string[] versions = check centralApiClient -> get("registry/packages/bctestorg/normal_package?include-prereleases=true");
    test:assertEquals(versions[0], "0.1.0");

    APIDocResponse apiDocsResp = check centralApiClient -> get("docs/bctestorg/normal_package/0.1.0");
